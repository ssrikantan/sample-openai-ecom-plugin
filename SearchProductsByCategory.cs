...
{
    public class SearchProductsByCategory
    {
        private readonly ILogger _logger;
        private static List<Product> allProducts;

        public SearchProductsByCategory(ILoggerFactory loggerFactory)
        {
        ...
        [OpenApiOperation(operationId: "SearchProductsByCategory", tags: new[] { "ExecuteFunction" }, Description = "Search for products based on their category.")]
        ...
        public HttpResponseData Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req)
        {
            ...
            else
            {
                ...
                var _products = allProducts.Where(c => c.Category.ToLower().Contains(category.ToLower())).ToList();
                var _productsAsString = string.Empty;
                foreach (var item in _products)
                {
                    _productsAsString += item.ProductName + " - " + item.ProductDescription + "\n";
                }
